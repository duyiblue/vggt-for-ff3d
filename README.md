...
- We extended `vggt/heads/dpt_head.py` to support alternative upsampling strategies (particularly learnable strategies). This is tailored for tasks where bilinear interpolation is undesirable (e.g., predicting Gaussian parameters for reconstruction). Concretely, the fused feature map has spatial size `(8·H/P, 8·W/P)` where `P` is the ViT patch size. We now support upsampling from `(8·H/P, 8·W/P)` to `(H, W)` using either PixelShuffle or ConvTranspose2d followed by a stride-8 downscale. You can also optionally increase the post-upsampling head capacity via `post_upsample_layers` and `post_upsample_width`. Rationale: linear mixing across pixels can corrupt structured parameters like rotation quaternions; a learnable upsampler avoids that averaging. Backward compatibility: if the new optional arguments are not provided, the head behaves exactly as before (final bilinear resize and the same head capacity).
